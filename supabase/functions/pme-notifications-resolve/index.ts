...
Deno.serve(async (request) => {
  ...
  await auth.supabase.from("audit_logs").insert({
    ...
    action: "pme_notifications.resolved",
    entity_table: "pme_notifications",
    entity_id: body.notificationId,
    metadata: { oldStatus: notification.row.status }
  });
  return jsonResponse({ id: body.notificationId, status: "resolved" });
});
