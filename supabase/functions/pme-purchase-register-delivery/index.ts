...
Deno.serve(async (request) => {
  ...
  if (order.row.status === "cancelled") {
    return jsonResponse({ error: "Cancelled purchase orders cannot receive deliveries." }, 400);
  }
  ...
  if (error !== null || !isIdRow(data)) {
    return jsonResponse({ error: "Could not register delivery." }, 400);
  }
  await auth.supabase
    .from("pme_purchase_orders")
    .update({ status: body.status === "complete" ? "delivered" : "partially_delivered" })
    .eq("id", order.row.id);
  await auth.supabase.from("audit_logs").insert({
    organization_id: order.row.organization_id,
    actor_user_id: auth.userId,
    action: "pme_purchase.delivery_registered",
    entity_table: "pme_purchase_deliveries",
    entity_id: data.id,
