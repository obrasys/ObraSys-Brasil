...
Deno.serve(async (request) => {
  ...
  if (!isRequestBody(body) || !body.reason.trim()) {
    return jsonResponse({ error: "Reopen reason is required." }, 400);
  }
  ...
  await auth.supabase
    ...
    .update({
      ...
      reopened_at: now,
      closeout_notes: body.reason
    })
    .eq("id", body.closeoutId)
    .eq("project_id", body.projectId);
  await auth.supabase.from("projects").update({ status: "active" }).eq("id", body.projectId);
  await auth.supabase.from("audit_logs").insert({
    organization_id: closeout.row.organization_id,
    actor_user_id: auth.userId,
    action: "pme_project.reopened",
    entity_table: "projects",
    entity_id: body.projectId,
    metadata: { closeoutId: body.closeoutId, reason: body.reason }
  });
  return jsonResponse({ status: "reopened", reopenedAt: now });
});
...
> {
  ...
  if (role.error !== null || role.data !== true) {
    return { ok: false, error: "User is not allowed to reopen this project.", status: 403 };
  }
