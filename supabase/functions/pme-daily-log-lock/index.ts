...
Deno.serve(async (request) => {
  ...
  if (!isRequestBody(body)) return jsonResponse({ error: "Invalid lock payload." }, 400);
  ...
  if (!allowed.ok) return jsonResponse({ error: allowed.error }, 403);
  ...
  const { data, error } = await auth.supabase
    ...
    .update({ status: "locked", locked_by: auth.userId, locked_at: lockedAt })
  ...
  await auth.supabase.from("audit_logs").insert({
    ...
    action: "pme_daily_log.locked",
    entity_table: "pme_project_daily_logs",
    entity_id: dailyLog.row.id,
    metadata: { locked_at: lockedAt }
  });
  return jsonResponse({ id: data.id, status: "locked" });
});
...
async function assertManager(
  ...
  return error === null && data === true
    ? { ok: true }
    : { ok: false, error: "User is not allowed to lock this daily log." };
}
