...
Deno.serve(async (request) => {
  ...
  if (!isRequestBody(body)) {
    return jsonResponse({ error: "Invalid notification generation payload." }, 400);
  }
  ...
  await auth.supabase.from("audit_logs").insert({
    ...
    action: "pme_notifications.generated_manually",
    entity_table: "pme_notifications",
    entity_id: null,
    metadata: { runMode: body.runMode }
  });
...
async function resolveManagerOrganization(
  ...
  if (role.error !== null || role.data !== true) {
    return { ok: false, error: "User is not allowed to generate PME notifications.", status: 403 };
  }
...
function isRequestBody(value: unknown): value is RequestBody {
  return (
    ...
    (value.runMode === "manual" || value.runMode === "digest")
  );
}
